...
LEARNING_FACTORY_OUTPUT_DIR（CLI --output-dir 的落地通道）覆盖，
...
def test_enforcement_honors_output_dir_env(monkeypatch):
    ...
    monkeypatch.setenv("LEARNING_FACTORY_OUTPUT_DIR", "MyFactory")
...
def test_enforcement_defaults_to_learning_factory(monkeypatch):
    """未设置环境变量时保持默认目录（防回归）。"""
    monkeypatch.delenv("LEARNING_FACTORY_OUTPUT_DIR", raising=False)
    text = agent.load_skills()
